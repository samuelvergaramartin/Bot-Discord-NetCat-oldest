...
var ncomando = listallcommands.setwelcomemessage;
var estadomodulo = estadomodulosdb.configuracion;
var estadocomando = estadocomandosdb.setwelcomemessage;
...
module.exports = {
    ...
    run: async(netcat, message, args) => {
        ...
        if(!perms) return message.channel.send("**:x: | ERROR:** No tienes los suficientes permisos.\nPermisos que te faltan: `Administrator`");
            ...
            var welcomemessage = await welcomemmessagedb.obtener(`${message.guild.id}`);
                if(welcomemessage) {
                   
                    if(args.join(" ").length > 1) {
                        var mensajedebienvenida = args.join(" ");
                        welcomemmessagedb.set(`${message.guild.id}`, `${mensajedebienvenida}`);
                        message.channel.send(`Se ha establecido el mensaje de bienvenida correctamente.\nLectura del mensaje de bienvenida:\n${mensajedebienvenida}`);
                    }
                    if (args.join(" ").length < 1) {
                        message.channel.send("Debes decir un mensaje de bienvenida.\nSintaxis: `nc/set-welcome-message` <mensaje de bienvenida>\nPor ejemplo: `nc/set-welcome-message` Hola michurri! Bienvenid@ al servidor!\nNota: en cada mensaje de bienvenida al principio siempre se mencionará al nuevo usuario, y eso no se puede cambiar ni quitar.");
