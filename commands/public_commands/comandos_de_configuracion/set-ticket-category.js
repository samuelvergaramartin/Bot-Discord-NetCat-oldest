...
var ncomando = listallcommands.setticketcategory;
var estadomodulo = estadomodulosdb.configuracion;
var estadocomando = estadocomandosdb.setticketcategory;
    ...
    run: async(netcat, message, args) => {
        ...
        async function ejecutarcomandoisOK() {
            async function ejecutarcomando() {
                var categoriamencionada = args[0];
                var categoriastickets = await categoriasticketsdb.obtener(`${message.guild.id}`);
                if(categoriamencionada) {
                    
                }
                if(categoriastickets) {
                    message.channel.send("**:warning: | Aviso:** Ya existe una categoría.")              
                 }
            }
