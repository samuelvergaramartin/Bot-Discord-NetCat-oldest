...
var ncomando = listallcommands.setwelcomechannel;
var estadomodulo = estadomodulosdb.configuracion;
var estadocomando = estadocomandosdb.setwelcomechannel;
        ...
        async function ejecutarcomandoisOK() {
            async function ejecutarcomando() {
               ...
               /* if (!message.guild.members.me.permissions.has("Administrator")) {
                  return message.channel.send("**❌ | ERROR: **No tengo los permisos necesarios.\n Permisos que me faltan: `Administrator`")
                  }*/
                if(!perms) return message.channel.send("**:x: | ERROR:** No tienes los suficientes permisos.\nPermisos que te faltan: `Administrator`");
                 else {
                    var welcomesystem = await welcomesystemdb.obtener(`${message.guild.id}`);
                  var nombredelcanalaestablecer = message.mentions.channels.first();
                 if(!nombredelcanalaestablecer) {
                    message.channel.send("**:x: | ERROR:** Debes mencionar un canal!\nSintaxis: `nc/set-welcome-channel` <mención del canal>\nPor ejemplo: `nc/set-welcome-channel` #michurri");
                    }
                    if(nombredelcanalaestablecer) {
                        if(welcomesystem) {
                   
                            message.channel.send(`**:warning: | Aviso:** Ya hay un canal de bienvenidas establecido en este servidor. Estás seguro de que deseas cambiarlo por el canal ${nombredelcanalaestablecer} ?\n(y/n)`).then((msg) => {
                                let filtro = "no respondida";
                                netcat.on("messageCreate", async (message) =>{
                                    if(filtro === "no respondida") {
                                        if(message.content === "y") {
                                            welcomesystemdb.set(`${message.guild.id}`, `${nombredelcanalaestablecer.id}`);
                                            message.channel.send(`Se ha establecido el canal ${nombredelcanalaestablecer} como canal de bienvenidas en este servidor.`);
                                            filtro = "respondida";
                                            
                                        }
                                        if(message.content === "n") {
                                            message.channel.send("Operación cancelada.")
                                            filtro = "respondida";
                                        }
                                    }
