...
var ncomando = listallcommands.setgoodbyemessage;
var estadomodulo = estadomodulosdb.configuracion;
var estadocomando = estadocomandosdb.setgoodbyemessage;
...
module.exports = {
    ...
    run: async(netcat, message, args) => {
        ...
        if(!perms) return message.channel.send("**:x: | ERROR:** No tienes los suficientes permisos.\nPermisos que te faltan: `Administrator`");
            ...
            var goodbyemessage = await goodbyemessagedb.obtener(`${message.guild.id}`);
            if(goodbyemessage) {
               
                if(args.join(" ").length > 1) {
                    var mensajededespedida = args.join(" ");
                    goodbyemessagedb.set(`${message.guild.id}`, `${mensajededespedida}`);
                    message.channel.send(`Se ha establecido el mensaje de despedida correctamente.\nLectura del mensaje de despedida:\n${mensajededespedida}`);
                }
                if (args.join(" ").length < 1) {
                    message.channel.send("Debes decir un mensaje de despedida.\nSintaxis: `nc/set-goodbye-message` <mensaje de despedida>\nPor ejemplo: `nc/set-goodbye-message` Adios michurri! Te echaremos de menos! Gracias por haber estado con nosotr@s!\nNota: en cada mensaje de despedida al principio siempre se dirá el nombre del usuario que ha abandonado el servidor, y eso no se puede cambiar ni quitar.");
            ...
            if(!goodbyemessage) {
                if(args.join(" ").length > 1) {
                    var mensajededespedida = args.join(" ");
                    goodbyemessagedb.set(`${message.guild.id}`, `${mensajededespedida}`);
                    message.channel.send(`Se ha establecido el mensaje de despedida correctamente.\nLectura del mensaje de despedia:\n${mensajededespedida}`);
                }
                if (args.join(" ").length < 1) {
                    message.channel.send("Debes decir un mensaje de despedida.\nSintaxis: `nc/set-goodbye-message` <mensaje de despedida>\nPor ejemplo: `nc/set-goodbye-message` Adios michurri! Te echaremos de menos! Gracias por haber estado con nosotr@s!\nNota: en cada mensaje de despedida al principio siempre se dirá el nombre del usuario que ha abandonado el servidor, y eso no se puede cambiar ni quitar.");
