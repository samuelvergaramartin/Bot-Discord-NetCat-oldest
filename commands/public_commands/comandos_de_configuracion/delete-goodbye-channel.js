...
var ncomando = listallcommands.deletegoodbyechannel;
var estadomodulo = estadomodulosdb.configuracion;
var estadocomando = estadocomandosdb.deletegoodbyechannel;
            ...
            var goodbyesystem = await goodbyesystemdb.obtener(`${message.guild.id}`);
                if(goodbyesystem) {
                    if(goodbyesystem === "ninguno") {
                        message.channel.send("**:x: | ERROR:** Este servidor no tiene ningún canal de despedidas. Si desea uno, use el comando `nc/set-goodbye-channel`.");
                    }
                    if(goodbyesystem !== "ninguno") {
                        goodbyesystemdb.set(`${message.guild.id}`, "ninguno");
                        message.channel.send("Sistema de despedidas eliminado correctamente en este servidor. Si desea volver a crearlo, use el comando `nc/set-goodbye-channel`.");
