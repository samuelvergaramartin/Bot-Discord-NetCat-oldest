...
var ncomando = listallcommands.deletewelcomechannel;
var estadomodulo = estadomodulosdb.configuracion;
var estadocomando = estadocomandosdb.deletewelcomechannel;
        ...
        if(!perms) return message.channel.send("**:x: | ERROR:** No tienes los suficientes permisos.\nPermisos que te faltan: `Administrator`");
            ...
            if(welcomesystem) {
                if(welcomesystem === "ninguno") {
                    message.channel.send("**:x: | ERROR:** Este servidor no tiene ningún canal de bienvenida. Si desea uno, use el comando `nc/set-welcome-channel`.");
                }
                if(welcomesystem !== "ninguno") {
                    welcomesystemdb.set(`${message.guild.id}`, "ninguno");
                    message.channel.send("Sistema de bienvenidas eliminado correctamente en este servidor. Si desea volver a crearlo, use el comando `nc/set-welcome-channel`.");
