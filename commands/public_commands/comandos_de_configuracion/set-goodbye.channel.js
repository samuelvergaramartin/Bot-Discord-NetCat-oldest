...
var ncomando = listallcommands.setgoodbyechannel;
var estadomodulo = estadomodulosdb.configuracion;
var estadocomando = estadocomandosdb.setgoodbyechannel;
        ...
        async function ejecutarcomandoisOK() {
            async function ejecutarcomando() {
                ...
                if(!perms) return message.channel.send("**:x: | ERROR:** No tienes los suficientes permisos.\nPermisos que te faltan: `Administrator`");
                    ...
                    var goodbyesystem = await goodbyesystemdb.obtener(`${message.guild.id}`);
                  var nombredelcanalaestablecer = message.mentions.channels.first();
                 if(!nombredelcanalaestablecer) {
                    message.channel.send("**:x: | ERROR:** Debes mencionar un canal!\nSintaxis: `nc/set-goodbye-channel` <mención del canal>\nPor ejemplo: `nc/set-goodbye-channel` #michurri");
                    }
                    if(nombredelcanalaestablecer) {
                        if(goodbyesystem) {
                   
                            message.channel.send(`**:warning: | Aviso:** Ya hay un canal de despedidas establecido en este servidor. Estás seguro de que deseas cambiarlo por el canal ${nombredelcanalaestablecer} ?\n(y/n)`).then((msg) => {
                                let filtro = "no respondida";
                                netcat.on("messageCreate", async (message) =>{
                                    if(filtro === "no respondida") {
                                        if(message.content === "y") {
                                            goodbyesystemdb.set(`${message.guild.id}`, `${nombredelcanalaestablecer.id}`);
                                            message.channel.send(`Se ha establecido el canal ${nombredelcanalaestablecer} como canal de despedidas en este servidor.`);
                                            filtro = "respondida";
