...
var ncomando = listallcommands.spank;
var estadomodulo = estadomodulosdb.reaccion;
var estadocomando = estadocomandosdb.spank;
    ...
    run: async(netcat, message, args) => {
        ...
        async function ejecutarcomandoisOK() {
            async function ejecutarcomando() {
                ...
                if(!user) return message.channel.send(":x: || D- debes mencionar a al- alguien") || netcat.users.cache.get(args[0]) || message.author;
                const { EmbedBuilder } = require('discord.js');
              const spank = new EmbedBuilder()
