...
var ncomando = listallcommands.addstaffrole;
var estadomodulo = estadomodulosdb.moderacion;
var estadocomando = estadocomandosdb.addstaffrole;
var modulodeestecomando = moderacion;
const serversmodstatusdb = new db.crearDB('serversmodstatus');
const staffrole = new db.crearDB('ticketsstaffrole');
    ...
    run: async(netcat, message, args) => {
        ...
        async function ejecutarcomandoisOK() {
            async function ejecutarcomando() {
                var perms = message.member.permissions.has("ManageRoles")
                if(!perms) return message.channel.send("**:x: | ERROR:** No tienes los suficientes permisos.\nPermisos que te faltan: `ManageRoles`")
                if (!message.guild.members.me.permissions.has("ManageRoles")) {
                  return message.channel.send("**❌ | ERROR: **No tengo los permisos necesarios.\n Permisos que me faltan: `ManageRoles`.")
                  }
                  let rol = message.mentions.roles.first();
                  if(!rol) return message.channel.send(":warning: | **Debes mencionar un rol**.\nSintaxtis: `nc/add-staff-role` <mención del rol>.\nPor ejemplo: `nc/add-staff-role` @michurri");
                
                  staffrole.establecer(message.guild.id, rol.id)
                const mum = new Discord.EmbedBuilder()
                    .setTitle("Rol del Staff para el sistema de tickets")
                    .setDescription(`Se ha establecido el rol **${rol.name}** como el rol de staff para atender los tickets de soporte en este servidor.`)
                    .setColor('0xff00d9')
