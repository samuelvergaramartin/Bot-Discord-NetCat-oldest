...
var ncomando = listallcommands.newticket;
var estadomodulo = estadomodulosdb.moderacion;
var estadocomando = estadocomandosdb.newticket;
...
module.exports = {
    ...
    run: async(netcat, message, args) => {
        ...
        if(!perms) return message.channel.send("**:x: | ERROR:** No tienes los suficientes permisos.\nPermisos que te faltan: `Administrator`");
                    ...
                    .setTitle("Tickets de soporte de NetCat.")
                ...
                if(prefix === "pnc/") {
                    var cliente = "client";
                    var id = "test-client-ticket";
                    anticrashticketsdb.set(`${cliente}.${message.guild.id}.${id}`);
                    ejecutarcodigo();
                }
                else {
                    var cliente = "client";
                    var id = "client-ticket";
                    anticrashticketsdb.set(`${cliente}.${message.guild.id}`, `${id}`);
                    ejecutarcodigo();
                }
