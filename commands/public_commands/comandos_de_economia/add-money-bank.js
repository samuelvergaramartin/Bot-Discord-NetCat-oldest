...
var ncomando = listallcommands.addmoneybank;
var estadomodulo = estadomodulosdb.economia;
var estadocomando = estadocomandosdb.addmoneybank;
        ...
        async function ejecutarcomandoisOK() {
            async function ejecutarcomando() {
              ...
              if(!persona) return message.channel.send("Te falta mencionar a alguien.\nSintaxsis: `nc/add-money-bank` <cantidad> <@usuario>")
                 ...
                 banco.sumar(`${persona.id}`, cantidadd)
            
                 return message.channel.send(`Has añadido **${cantidadd}**:coin: a la cuenta bancaria de **${persona.tag}**`)
                 }
              /*if(cantidadd === 'all'){
                const cantidadpersona = await dinero.obtener(`${persona.id}`)
