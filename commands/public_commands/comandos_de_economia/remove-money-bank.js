...
var ncomando = listallcommands.removemoneybank;
var estadomodulo = estadomodulosdb.economia;
var estadocomando = estadocomandosdb.removemoneybank;
...
module.exports = {
    ...
    if(!persona) return message.channel.send("Te falta mencionar a alguien.\nSintaxsis: `nc/remove-money-bank` <cantidad> <@usuario>")
       ...
       banco.restar(`${persona.id}`, cantidadd)
  
       return message.channel.send(`Has quitado **${cantidadd}**:coin: de la cuenta bancaria de **${persona.tag}**`)
       }
    /*if(cantidadd === 'all'){
      const cantidadpersona = await dinero.obtener(`${persona.id}`)
